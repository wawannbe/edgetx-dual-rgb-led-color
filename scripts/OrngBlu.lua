local function init()
end

local function run()
    -- Right gimbal
    for i = 0, LED_STRIP_LENGTH /2, 1
    do
        setRGBLedColor(i, 0, 0, 255) -- Blue
    end

    -- Right set of RGB switches
    setCFSLedColor("SW4", 0, 0, 255)
    setCFSLedColor("SW5", 0, 0, 255)
    setCFSLedColor("SW6", 0, 0, 255)

    -- Left gimbal
    for j = LED_STRIP_LENGTH /2  -3, LED_STRIP_LENGTH, 1
    do
        setRGBLedColor(j, 255, 165, 0) -- Orange
    end

    -- Left set of RGB switches
    setCFSLedColor("SW1", 255, 165, 0)
    setCFSLedColor("SW2", 255, 165, 0)
    setCFSLedColor("SW3", 255, 165, 0)
    
    applyRGBLedColors()
end

local function background()
end

return {run = run, background = background, init = init}